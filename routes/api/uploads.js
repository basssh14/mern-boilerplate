...
router.post('/parents/cnicphotos', auth, async(req, res) => {
    try {
        if (!req.files) {
            ...
            cnicCopy.mv(__dirname + './client/public/img/parents/cnic/' + cnicCopy.name);
...
router.post('/parents/salaryphotos', auth, async(req, res) => {
    try {
        if (!req.files) {
            ...
            //Use the mv() method to place the file in upload directory (i.e. "uploads")
            salarySlip.mv('./client/public/img/parents/salary/' + salarySlip.name);
...
router.post('/parents/qualiDocphotos', auth, async(req, res) => {
    try {
        if (!req.files) {
            ...
            //Use the mv() method to place the file in upload directory (i.e. "uploads")
            qualiDoc.mv('./client/public/img/parents/quali/' + qualiDoc.name);
